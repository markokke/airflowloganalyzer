...
class LogAnalyzer:
    def __init__(
        self, config: Dict, report_generator: Optional[ReportGenerator] = None
    ):
        ...
        try:
            ...
            for name, pattern in self.config["analysis"]["patterns"].items():
                ...
                if "regex" not in pattern:
                    self.logger.warning(
                        f"Pattern '{name}' is missing regex configuration"
                    )
                    continue
                self.patterns[name] = pattern["regex"]
                self.logger.debug(
                    f"Initialized pattern '{name}' with regex: {pattern['regex']}"
                )
    ...
    def _init_mongodb(self):
        ...
        try:
            self.logger.debug(
                f"Attempting to connect to MongoDB at {mongo_config['uri']}"
            )
            ...
            for idx_config in text_index_configs:
                ...
                if needs_recreation:
                    ...
                    self.log_collection.create_index(
                        ...
                        language_override=idx_config.get(
            ...
            for index in index_config:
                ...
                if index_name not in existing_index_names:
                    ...
                    self.analysis_collection.create_index(
                        [(field, direction)], background=True
                    )
    ...
    def _verify_collections(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Found {analysis_count} documents in analysis collection"
            )
    ...
    def analyze_individual_logs(self) -> List[Dict]:
        ...
        try:
            ...
            has_text_index = any(
                ...
                and "LogInfo" in idx.get("weights", {})
                for idx in indexes
            )
            ...
            try:
                logs = self.log_collection.find(
                    query, batch_size=self.config["mongodb"]["batch_size"]
                )
                ...
                ollama_client = OllamaClient(
                    model_name=self.config["model"]["name"], config=self.config
                )
                ...
                for log in logs:
                    try:
                        ...
                        self.logger.info(
                            f"Starting analysis of log {log_count} of {total_logs} (ID: {log_id})"
                        )
                        ...
                        for pattern_name, pattern_config in self.config["analysis"][
                            "patterns"
                        ].items():
                        ...
                        try:
                            ...
                            self.logger.error(
                                f"Error during Ollama analysis for log {log_id}: {str(e)}"
                            )
                        ...
                        analysis_data = {
                            ...
                            "recommendations": self._generate_recommendations(
                                dict(pattern_results)
                            ),
                            "summary": {
                                "log_id": log_id,
                                "timestamp": str(log["_id"].generation_time),
                                "total_issues_found": sum(
                                    len(matches) for matches in pattern_results.values()
                                ),
                            },
                        }
                        ...
                        try:
                            ...
                            self.logger.info(
                                f"Stored analysis in MongoDB with ID: {analysis_id}"
                            )
                        except Exception as e:
                            self.logger.error(
                                f"Error storing analysis in MongoDB: {str(e)}"
                            )
                        ...
                        try:
                            formatted_report = self.report_generator.format_analysis(
                                log_analysis, analysis_data
                            )
                            filename = f"airflow_analysis_log_{log_id}.txt"
                            report_path = self.report_generator.save_report(
                                formatted_report, filename
                            )
                            self.logger.info(
                                f"Generated report for log {log_id}: {report_path}"
                            )
                        except Exception as e:
                            self.logger.error(
                                f"Error generating report for log {log_id}: {str(e)}"
                            )
                        ...
                        self.logger.error(
                            f"Error processing log {log_count} (ID: {log_id}): {str(e)}"
                        )
    ...
    def _extract_pattern(
        self, pattern_name: str, pattern_config: Dict, log_info: str, results: Dict
    ):
        ...
        for line in lines:
            ...
            current_timestamp = datetime.strptime(
                timestamp_match.group(1), "%Y-%m-%dT%H:%M:%S.%f%z"
            )
            ...
            for match in matches:
                ...
                if pattern_config.get("include_context"):
                    content = self._get_context(
                        line, match, self.config["analysis"]["context_lines"]
                    )
                else:
    ...
    def _analyze_pod_events(self, log_content: str) -> List[Dict]:
        ...
        for match in re.finditer(pod_pattern, log_content):
            ...
            events.append(
                {"pod_name": pod_name, "event_type": event_type, "status": status}
            )
            ...
            if isinstance(severities, dict):
                for severity, issues in severities.items():
                    if (
                        ...
                        recommendation = self._get_recommendation_for_pattern(
                        ...
                        recommendations.append(
                            {
                                ...
                                "sample": (
                                    issues[0]["content"][:200] if issues else None
                                ),
                            }
                        )
