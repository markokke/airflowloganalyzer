...
class ReportGenerator:
    ...
    def _get_output_path(self, filename: Union[str, Path], subdir: str = None) -> Path:
        ...
        if subdir:
            base_dir = base_dir / self.output_config['subdirectories'].get(subdir, subdir)
    ...
    def save_outputs(self, analysis: Dict, model_request: Dict):
        ...
        if self.output_config.get('save_analysis'):
            json_path = self._get_output_path(
                self.output_config['analysis_file'], 
                subdir='json'
            )
        ...
        if self.output_config.get('save_model_request'):
            request_path = self._get_output_path(
                self.output_config['model_request_file'], 
                subdir='json'
            )
            self.logger.debug(f"Saving model request to {request_path}")
            self._save_json(model_request, request_path)

    def save_report(self, formatted_analysis: str, filename: Optional[str] = None) -> Path:
    ...
    def generate_individual_reports(self, analysis_results: List[Dict]) -> List[Path]:
        ...
        for result in analysis_results:
            ...
            analysis_data = {
                ...
                'recommendations': self._generate_recommendations(result['pattern_matches'])
            }
            
            # Format the report
            formatted_report = self.format_analysis(
                result['analysis'],
                analysis_data
            )
            
            # Generate unique filename for this log
            timestamp_str = datetime.fromisoformat(str(timestamp).replace('Z', '+00:00')).strftime('%Y%m%d_%H%M%S')
            ...
            if matches:  # If there are matches for this pattern
                ...
                for severity, count in severity_counts.items():
                    recommendations.append({
                        ...
                        'severity': severity,
                        'count': count,
                        'suggestion': self._get_recommendation_for_pattern(pattern_type, severity)
                    })
    ...
    def _get_recommendation_for_pattern(self, pattern_type: str, severity: str) -> str:
        ...
        return pattern_recs.get(severity, f'Review {pattern_type} issues with {severity} severity')     
    ...
    def format_analysis(self, model_response: Dict, analysis_data: Dict) -> str:
        ...
        if 'log_info' in analysis_data:
            ...
            if 'AIRFLOW_CTX_DAG_RUN_ID=' in log_content:
                try:
                    dag_run_id = log_content.split("AIRFLOW_CTX_DAG_RUN_ID='")[1].split("'")[0]
                    self.logger.debug(f"Found DAG RUN ID in pattern match: {dag_run_id}")
                except (IndexError, AttributeError):
                    self.logger.debug("Failed to extract DAG RUN ID from raw log")
                    
            if 'AIRFLOW_CTX_TASK_ID=' in log_content:
                try:
                    task_id = log_content.split("AIRFLOW_CTX_TASK_ID='")[1].split("'")[0]
            ...
            if 'AIRFLOW_CTX_TRY_NUMBER=' in log_content:
                try:
                    try_number = log_content.split("AIRFLOW_CTX_TRY_NUMBER='")[1].split("'")[0]
                    self.logger.debug(f"Found TRY NUMBER in pattern match: {try_number}")
                except (IndexError, AttributeError):
            ...
            for pattern_type, matches in analysis_data['patterns'].items():
                for match in matches:
                    ...
                    if 'AIRFLOW_CTX_DAG_ID=' in content:
                        try:
                            dag_id = content.split("AIRFLOW_CTX_DAG_ID='")[1].split("'")[0]
                            self.logger.debug(f"Found DAG ID in pattern match: {dag_id}")
                            break
                        except (IndexError, AttributeError):
                            continue
        ...
        if dag_id == "Unknown":
            self.logger.warning(f"Could not extract DAG ID for log {analysis_data['summary']['log_id']}")
            self.logger.debug("Available patterns: " + ", ".join(analysis_data['patterns'].keys()))
        
        # Add Pattern Matches Section
        if analysis_data['patterns']:
            formatted.extend([
                "-" * 40,
                "DETECTED PATTERN SNIPPETS",
                "-" * 40,
                ""
            ])
        ...
        if model_response:
            ...
            for analysis_type, analysis in model_response.items():
                if analysis and isinstance(analysis, dict) and 'response' in analysis:
                    ...
                    self.logger.warning(f"Unexpected analysis format for {analysis_type}: {analysis}")
                    formatted.extend([
                        f"{analysis_type.upper().replace('_', ' ')}:",
                        "-" * len(analysis_type),
