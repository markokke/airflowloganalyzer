...
from prompts import (AIRFLOW_LOG_ANALYSIS_PROMPT, DBT_ANALYSIS_PROMPT,
                     ERROR_ANALYSIS_PROMPT, PERFORMANCE_ANALYSIS_PROMPT,
...
class OllamaClient:
    ...
    def _make_request(
        ...
        endpoint: str,
        data: Dict = None,
        stream: bool = False,  # Note the trailing comma here
        ...
        for attempt in range(self.retries):
            try:
                ...
                )  # Note the trailing comma on the last parameter and closing paren on new line
                response.raise_for_status()
                return response if stream else response.json()
            except requests.RequestException as e:
                if attempt == self.retries - 1:  # Last attempt
                    print(
                        f"Failed to connect to Ollama after {self.retries} attempts: {str(e)}"
                    )
                    raise
    ...
    def create_model(self, model_request: Dict, force_recreate: bool = False) -> Dict:
        ...
        try:
            response = self._make_request(
                "POST", "api/create", model_request, stream=True
            )
            status_updates = []
    ...
    def ask_model(
        self, log_content: str, prompt_type: str = "main", log_id: str = None
    ) -> Dict:
        ...
        formatted_prompt = self._format_prompt(
            prompt_template, log_content, prompt_type
        )
        ...
        prompt_file = (
            prompts_dir / f"prompt_{prompt_type}{log_id_part}_{timestamp}.json"
        )
